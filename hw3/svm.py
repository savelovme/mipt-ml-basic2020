...
def rbf(x_1, x_2, sigma=1.):
    ...
    distances = torch.exp(- distances / (2 * sigma**2))
...
def hinge_loss(scores, labels):
    ...
    y = (1 - scores * labels)[(1 - scores * labels) > 0]
    return torch.sum(y) / scores.shape[0]
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = self.bias + k_batch @ self.betas### YOUR CODE HERE # get the matrix product using SVM parameters: self.betas and self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return (self.bias + K @ self.betas)[:, 0]
